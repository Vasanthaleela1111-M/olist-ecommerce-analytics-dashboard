...
Python Version License Dashboard Status Data Grain

An enterprise-grade, evidence-based data analytics platform and interactive web dashboard built on the Brazilian Olist E-Commerce dataset (
N
=
99
,
441
 orders, 
R
 15.84\text{M}$ GMV across 2016–2018).
...
Total Analyzed Volume	99,441 Orders	
N
=
96
,
470
 Delivered Orders evaluated for logistics & ratings
...
Inject Route Buffer Days (
0
 to 
+
7
 days): Extends estimated delivery promise windows to eliminate false lateness penalties.
Max Seller Dispatch SLA Cap (
1
 to 
10
 days): Enforces carrier handoff SLA caps to eliminate seller warehouse lag.
Mathematical Delay Reduction Engine: Computes exact order-level net delay 
D
′
=
D
...
dispatchSLA
 ...
 across all 99,441 orders in real-time.
...
Q10 Seller Health Scorecard: Automated grading system (A+ to F) evaluating sellers on dispatch SLA compliance (
≤
3
 days), late delivery rate, and low rating percentage with automated badging (Platinum Preferred, Silver At-Risk, Terminated).
📊 Core Analytical Findings
Marketplace Volume Scaling vs. Rating Lockstep:

Order volume expanded +714% (from 788 to 7,380 peak monthly orders) without overall review score degradation (r = -0.13).
Monthly average review score drops correlate strictly in lockstep with monthly late delivery rate spikes (
r
=
+
0.79
).
Delivery Lateness Multiplier:
...
≥
4
 days late) leads to 75.09% 1–2 star ratings (4.97x relative risk vs. baseline).
Geographic Concentration Friction:
...
Shipping corridor SP -> RJ (
N
=
8
,
431
) exhibits a 14.99% late delivery rate and 22.57% low rating rate.
Product Category Outliers:

Bulky category office_furniture (
N
=
1
,
264
) is the worst fulfillment outlier: 3.62 stars, 22.63% low rating rate, R$ 53.98 average freight (+136.5% vs baseline), and 20.71-day lead time.
